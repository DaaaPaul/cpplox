...
void AstPrinter::visitLiteral(Literal& literal) {
...
void AstPrinter::visitGrouping(Grouping& grouping) {
	parenthesize("grouping", { *(grouping.expr) });
}

void AstPrinter::visitUnary(Unary& unary) {
	parenthesize(unary.op.toLexeme(), { *unary.right });
}

void AstPrinter::visitBinary(Binary& binary) {
	parenthesize(binary.op.toLexeme(), { *binary.left, *binary.right });
}

void AstPrinter::parenthesize(std::string const& identity, std::initializer_list<std::reference_wrapper<Expr>> const& exprs) {
	std::cout << "(" << identity;
